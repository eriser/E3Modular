...
namespace e3 {
    ...
    ParameterSet& MidiFrequency::getDefaultParameters() const
    {
        ...
        const Parameter& paramBend = set.addModuleParameter( ParamBendRange, id_, "BendRange", ControlBiSlider, 2 );
        paramBend.valueShaper_ = { -48, 48, 96 };

        const Parameter& paramTime = set.addModuleParameter( ParamGlideTime, id_, "Portamento Time", ControlSlider, 0 );
        paramTime.valueShaper_ = { 0, 2000 };
        paramTime.unit_ = "msec";
        paramTime.numberFormat_ = NumberFloat;

        const Parameter& paramAuto = set.addModuleParameter( ParamGlideAuto, id_, "Portamento Auto", ControlCheckbox, 0 );
        paramAuto.valueShaper_ = { 0, 1 };
